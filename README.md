...
The built-in default config lives at [`src/canirunai/config/default_config.toml`](/home/kuseo/toy/can-i-run-ai/src/canirunai/config/default_config.toml). At runtime, canirunai loads that file first and then deep-merges a user-supplied config file on top of it.
